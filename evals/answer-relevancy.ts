...
// System instructions for the relevancy evaluator
...
const checkAnswerRelevancy = async (opts: {
  ...
  // Step 1: Break down the answer into statements
  const statementsResult = await generateObject({
  ...
  const statements = statementsResult.object.statements;
  ...
  if (statements.length === 0) {
    return {
      ...
      metadata: {
        statements: [],
        verdicts: [],
        reasoning: "No statements found in the submission",
      },
    };
  }
  ...
  const verdicts = evaluationResult.object.verdicts;
  ...
  return {
    ...
    metadata: {
      statements: statements,
      verdicts: verdicts,
      reasoning: `Evaluated ${statements.length} statements. Average relevancy score: ${averageScore.toFixed(2)}`,
    },
  };
};
